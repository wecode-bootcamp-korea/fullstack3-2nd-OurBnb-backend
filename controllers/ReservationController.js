...
const postReservation = async (req,res) => {
  try {
    ...
    console.log(err);
    return res.status(err.statusCode || 500).json(err.message ||  { message: 'POST_RESERVATION_FAILED' });
  }
}
...
const getReservation = async (req, res) => {
  try {
    ...
    console.log(err); 
    return res.status(err.statusCode || 500).json(err.message || { message: 'GET_RESERVATION_FAILED' }); 
  }
}
...
const updateReservation = async (req, res) => {
  try {
    ...
    const reservation = await reservationService.updateReservation(guestCount, oldCheckIn, oldCheckOut, newCheckIn, newCheckOut, userId, roomId);

    return res.status(200).json({ message: 'RESERVATION_UPDATED', reservation });
  } catch (err) {
    console.log(err);
    return res.status(err.statusCode || 500).json(err.message ||  { message: 'UPDATE_RESERVATION_FAILED' });
  }
}
...
const deleteReservation = async (req, res) => {
  try {
    ...
    const reservation = await reservationService.deleteReservation(checkIn, checkOut, userId, roomId);

    return res.status(200).json({ message: 'RESERVATION_CANCELLED', reservation }); 
  } catch (err) {
    console.log(err); 
    return res.status(err.statusCode || 500).json(err.message || { message: 'DELETE_RESERVATION_FAILED' }); 
  }
}
                                                                                                   
  module.exports = { postReservation, getReservation, updateReservation, deleteReservation };
>>>>>>> d3b502c (Add: Reservation CRUD)
